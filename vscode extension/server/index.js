...
app.use('/openDialogBoxFile', function(req, res, next) {
    if (serverOptions.openDialogBoxFile){
        serverOptions.openDialogBoxFile().then(file=>h.sendOk(req, res, file && file[0] || {}));
        return
    }
    next()
});

app.use('/errorMessage', function(req, res, next) {
    if (serverOptions.errorMessage){
        ...
        serverOptions.errorMessage(req.body.message);
        h.sendOk(req, res);
        return
    }
    next()
...
app.use('/infoMessage', function(req, res, next) {
    if (serverOptions.infoMessage){
        if (error = h.bodyValidation(req, {
            val: 'message',
            type: 'string'
        })){
        ...
        serverOptions.infoMessage(req.body.message, req.body.options).then(input=>h.sendOk(req, res, input || ''));
        return
    }
    next()
});

app.use('/inputMessage', function(req, res, next) {
    if (serverOptions.inputMessage){
        if (error = h.bodyValidation(req, {
            val: 'message',
            type: 'string'
        })){
        ...
        serverOptions.infoMessage(req.body.message, req.body.options || {});
        h.sendOk(req, res);
        return
    }
    next()
