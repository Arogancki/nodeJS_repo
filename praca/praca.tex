...
Nie oferuję tak efektywanej pracy jak technologia Node.js, jednak może okazać się lepszym wyborem jeśli zachodzi potrzeba wykorzystania specyficznego protokołu sieciowego, który nie został jeszcze zaimplementowany w Node.js.

\subsection(Ringojs)
Ostatnią wymienioną alternatywą jest platforma Ringojs.
Napisany został w języku java przez Hannesa Wallnöfer w 2010 roku.
Służy do tworzenia aplikacji w języku javascript po stronie serwera działających również w koncerpcji środowiska sterowanego zdarzeniami dzięki wykorzystaniu technologi Rhino, która zajmuje się obsługą wielowątkowości. 
...
Oznacza że zyskuje przewage w przypadku prostych narzędzi, natomiast nie gwarantuje takiej skalowalności jak Node.js.
...
trzymuje odpowiedź zawierająca pliki statyczne serwisu (plik html, css, js, zdjęcie w tle i favicon.}
\end{figure}
...
Przeprowadzone testy wydajnościowe pozwalają sprawdzić zachowanie serwisu przy różnej szerokości zakresie danych. 

\subsection{Mała ilość danych}
Testy dla bazdy danych przy 5 uzytkownikach, kazdy po 2 tablice po 3 zadania po 2 komentarze
Wydajnosc aplikacji:
czas logowania:
wyslanie odpowiedzi z informacjiami:
dodanie boardu
doddanie taska
dodanie kometarza
wyszukanie uzytkownika
czas przeslania odpowiedzi do uzytkownika:

\subsection{Średnia ilość danych}
Do bazy danych została dodana duża ilość danych: 500 userow kazdy po 10 tablic po 20 zadan z 5 komentarzami. 

\subsection{Duża ilość danych}
Do bazy danych została dodana duża ilość danych: 5000 userow kazdy po 50 tablic po 50 zadan z 10 komentarzami. 

\subsection{Podsumowanie testów}
Czy serwis spelnia funkcjonalnosc w roznych obicaizeniach czy korzystanie z serwisu jest komfortowe i satysfakcjonujace?


\section{Testy obciazeniowe serwera}
Przeprowadzone testy pozwalają określic zachowanie aplikacji w przypadku równoczesnej obsługi wielu przychodzących zapytań.

\subsection{Mała ilość danych}
Wyslanie 100 logowan uzytkownikow naraz i otrzymanie ich informacji

\subsection{Średnia ilość danych}
Wyslanie 1000 logowan uzytkownikow naraz i otrzymanie ich informacji

\subsection{Duża ilość danych}
Wyslanie 10000 logowan uzytkownikow naraz i otrzymanie ich informacji

\subsection{Podsumowanie testów}
Czy serwis spelnia funkcjonalnosc w roznych obicaizeniach czy korzystanie z serwisu jest komfortowe i satysfakcjonujace? Czy serwis jest skalowalny?
...
\textit{https://en.wikipedia.org/wiki/Twisted_(software)}
\bibitem{Ringojs}
