...
const isFilePathValidIfTrue= f => parsers.toBoleanOrString(f)===true ? isFilePathValid(f) : null
const isNumber= n => Number.isNaN(n) ? `${n} is not a number` : undefined
const isMoreThan= (n,k) => (n<k) ? `${n} is not more than ${k}` : undefined
const fileExistsIfTrue= f => parsers.toBoleanOrString(f)===true ? fileExists(f) : null
